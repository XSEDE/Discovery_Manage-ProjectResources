...
print STDOUT "<ActiveProjectResources Timestamp=\"$timestamp\">\n";
foreach (@output) {
   if ( $last ne $_->{'project_number'} ) {
      if ( $last ) {
         print STDOUT "    </ActiveResources>\n";
         print STDOUT "  </ActiveProject>\n";
      }
      print STDOUT "  <ActiveProject>\n";
      print STDOUT "    <project_number>$_->{'project_number'}</project_number>\n";
      print STDOUT "    <ActiveResources>\n";
      $last = $_->{'project_number'};
   }
   my $ResourceID = resource_name_to_resourceid($_->{'resource_name'});
   print STDOUT "      <ResourceID>$ResourceID</ResourceID>\n";
   $count++;
}
if ( $count > 0 ){
   print STDOUT "    </ActiveResources>\n";
...
print STDOUT "</ActiveProjectResources>\n";
close(STDOUT);
